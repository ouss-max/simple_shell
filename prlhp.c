...
int prm(void)
{
    ...
    {
        writecount = write(STDOUT_FILENO, prm, 2);
        if (writecount == -1)
            exit(0);
    }
    return (0);
...
/**
 * rd - reads stdin and stores it in a buffer
 *
...
/**
 * flp - finds the string to call execve on
...
/**
 * cblt - check if first user string is a built-in
 * @av: pointer to array of user of strings
...
/**
 * frk - create child process to execute based on user input
 * @av: pointer to array of user of strings
 * @buffer: pointer to user string
